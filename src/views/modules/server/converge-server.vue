      ...
      <el-table-column
        ...
        <template slot-scope="scope" >
          ...
          <el-tooltip content="查看详情" :open-delay="1500" :hide-after="5000">
            <el-button type="primary" icon="el-icon-document"  size="mini" @click="viewDetail(scope.row.carrierPsn)" circle></el-button>
          </el-tooltip>
...
<script>
  import AddOrUpdate from './converge-server-add-or-update'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/convergeCarrierList/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        this.$confirm(`确定进行['删除']操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/convergeCarrierList/delete'),
            method: 'post',
      ...
      viewDetail (id) {
        this.$router.push({ path: '/convergeDetail', query: { psn: id } })
      }
    }
  }
</script>
